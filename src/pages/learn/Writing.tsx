    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Luyện viết với AI chấm bài tự động
        </p>{" "}
      </div>{" "}
      {!activeLesson ? (
        /* Lesson List */ lessons.length > 0 ? (
...
setEssayFeedback(null);
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    {" "}
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold ${isCompleted ? "bg-green-500 text-white" : "bg-muted text-muted-foreground"}`}
                    >
                      {" "}
                      {isCompleted ? (
          ...
          <div className="text-center py-16 bg-card rounded-2xl border border-border">
            ...
            <p className="text-muted-foreground">
              Admin hãy thêm bài học trong trang quản trị.
            </p>{" "}
          </div>
        )
      ) : (
        /* Active Lesson */ <div className="space-y-6">
          {" "}
          <Button variant="ghost" onClick={() => setActiveLesson(null)}>
            {" "}
            ← Quay lại danh sách{" "}
          </Button>{" "}
          {activeLesson.content?.type === "fill-blank" &&
          activeLesson.content.sentences ? (
            /* Fill in the Blanks */ <div className="bg-card rounded-2xl border border-border p-8">
              {" "}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <p className="text-lg text-foreground flex-1">
                          ...
                          <input
                            ...
                            className={`w-32 mx-2 px-3 py-1 rounded-lg border text-center ${showFillResults ? (isCorrect ? "bg-green-50 border-green-500 text-green-700 dark:bg-green-950/30 dark:text-green-400" : "bg-red-50 border-red-500 text-red-700 dark:bg-red-950/30 dark:text-red-400") : "border-border focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"}`}
                            placeholder="..."
                          />{" "}
...
setShowFillResults(false);
          ...
          ) : activeLesson.content?.type === "essay" ||
            activeLesson.content?.prompt ? (
            /* Essay Writing */ <div className="bg-card rounded-2xl border border-border p-8">
              {" "}
              <h2 className="text-xl font-bold text-foreground mb-2">
                {activeLesson.title}
              </h2>{" "}
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                    {" "}
                    <div
                      className={`h-full rounded-full transition-all ${wordCount >= 50 ? "bg-green-500" : wordCount >= 30 ? "bg-accent" : "bg-muted-foreground"}`}
                      style={{
                  ...
                  <span
                    className={`text-sm ${wordCount >= 50 ? "text-green-500" : "text-muted-foreground"}`}
                  >
                    {" "}
                    {wordCount >= 50
                      ? "✓ Đủ độ dài"
                      : `Cần thêm ${50 - wordCount} từ`}{" "}
                  </span>{" "}
                </div>{" "}
                {/* Feedback */}{" "}
                {essayFeedback && (
                  <div className="p-4 rounded-xl bg-purple-50 border border-purple-200 dark:bg-purple-950/20 dark:border-purple-900">
                    ...
                    <h4 className="font-bold text-foreground mb-2 flex items-center gap-2">
                      {" "}
                      <Zap className="w-5 h-5 text-purple-500" /> AI
                      Feedback{" "}
                    </h4>{" "}
