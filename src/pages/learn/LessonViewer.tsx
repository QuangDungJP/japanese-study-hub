...
import {
  ArrowLeft, BookOpen, CheckCircle2, Clock, Dumbbell, Download,
  FileText, Film, Presentation, Sparkles, Target, Trophy, Zap,
} from 'lucide-react';
...
const isVideo = (m: Material) =>
  m.file_type?.toLowerCase().startsWith('video') || /\.(mp4|webm|mov)($|\?)/i.test(m.file_url);
...
const LessonViewer = () => {
  ...
  const [presenter, setPresenter] = useState<{ url: string; title: string } | null>(null);
  ...
  const handleComplete = async () => {
    ...
    const { error } = await supabase.from('completed_lessons').insert({
      user_id: user.id, lesson_id: lesson.id, score: null,
    });
    if (error) { toast({ title: 'Lỗi', description: error.message, variant: 'destructive' }); return; }
    setCompleted(true);
    const xp = lesson.xp_reward ?? 20;
    addXp(xp);
    toast({ title: `+${xp} XP`, description: 'Đã đánh dấu hoàn thành bài học.' });
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-5 sm:p-7 space-y-4">
          <div className="flex items-center gap-2 flex-wrap">
            {lesson.skill && <Badge className="bg-primary/10 text-primary border-0">{lesson.skill}</Badge>}
            {lesson.level && <Badge variant="outline">{lesson.level}</Badge>}
            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
              <Clock className="w-3.5 h-3.5" /> {lesson.duration_minutes || lesson.estimated_minutes || 15} phút
            </span>
            <span className="inline-flex items-center gap-1 text-xs text-amber-600">
              <Zap className="w-3.5 h-3.5" /> +{lesson.xp_reward ?? 20} XP
            </span>
          </div>
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold leading-tight">{lesson.title_vi || lesson.title}</h1>
            {lesson.title_vi && lesson.title && lesson.title !== lesson.title_vi && (
      ...
      {/* HTML / text content */}
      {(lesson.content_html || (lesson.content && typeof lesson.content === 'object' && (lesson.content as any).text)) && (
        <Card>
          <CardContent className="p-5 sm:p-7">
            <div
              className="prose prose-sm sm:prose-base dark:prose-invert max-w-none leading-relaxed"
              dangerouslySetInnerHTML={{
                __html: lesson.content_html || (lesson.content as any).text || '',
              }}
            />
          </CardContent>
        </Card>
            ...
            <div className="grid gap-2">
              {materials.map((m) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${pdf ? 'bg-red-500/10 text-red-600' : video ? 'bg-purple-500/10 text-purple-600' : 'bg-blue-500/10 text-blue-600'}`}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {pdf && (
                        <Button
                          ...
                          variant="default"
                          className="gap-1.5"
                          onClick={() => setPresenter({ url: m.file_url, title: m.title })}
                        >
                          <Presentation className="w-4 h-4" />
                          <span className="hidden sm:inline">Trình chiếu</span>
                        </Button>
                      )}
                      <Button asChild size="icon" variant="ghost" title="Tải xuống">
      ...
      {!completed && (
        <div className="flex justify-center pt-4">
          <Button size="lg" onClick={handleComplete} className="gap-2 rounded-full px-8">
