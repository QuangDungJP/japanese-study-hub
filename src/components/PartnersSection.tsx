...
export const PartnersSection = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setStartIndex((prev) => (prev + 1) % PARTNERS_CATALOG.length);
    }, 3500);
    return () => clearInterval(timer);
  }, [isHovered]);

  const handlePrev = () => {
    setStartIndex((prev) => (prev - 1 + PARTNERS_CATALOG.length) % PARTNERS_CATALOG.length);
  };
  ...
  const visiblePartners = [0, 1, 2, 3].map(
    (offset) => PARTNERS_CATALOG[(startIndex + offset) % PARTNERS_CATALOG.length]
  );
  ...
  return (
    <section className="py-12 bg-white dark:bg-slate-950 border-t border-b border-slate-100 dark:border-slate-800">
      <div className="container mx-auto px-4">
        ...
        <div
          className="max-w-6xl mx-auto overflow-hidden py-3"
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 items-center justify-items-center transition-all duration-500">
            {visiblePartners.map((partner, i) => (
              <a
                key={`${partner.id}-${startIndex}-${i}`}
                href={partner.websiteUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center justify-center h-20 w-44 md:w-52 transition-all duration-300 hover:scale-105"
                title={partner.name}
              >
                <img
                  src={partner.logoUrl}
