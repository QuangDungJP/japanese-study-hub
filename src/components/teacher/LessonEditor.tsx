...
const LessonEditor = ({ initialData, onSubmit, onCancel, isEditing }: LessonEditorProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...formData,
        title: formData.title || formData.title_vi || 'Bài học',
      };
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label className="text-xs text-muted-foreground">Tiêu đề EN / JP (Không bắt buộc)</Label>
                      <Input
                        value={formData.title}
                        onChange={(e) => updateField('title', e.target.value)}
                        placeholder="Tự động dùng Tiếng Việt nếu để trống"
                        className="font-medium"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-xs text-muted-foreground">Tên bài học (Tiếng Việt)</Label>
                      <Input
